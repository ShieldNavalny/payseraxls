...
Ключ матчинга: vendor + amount + год-месяц (YYYY-MM).
Дата списания по карте может отличаться от даты инвойса на несколько дней,
поэтому точный день исключён из ключа, только месяц.
"""
...
def _year_month(raw) -> str:
    """Возвращает 'YYYY-MM' из любого формата даты."""
...
def _make_key(date, vendor: str, amount: str) -> Tuple[str, str, str]:
    """(год-месяц, canonical_vendor, |amount|)"""
    return (
        _year_month(date),
...
def load_xls_keys(xls_path: Path) -> Set[Tuple[str, str, str]]:
    """(year-month, vendor, amount) только для расходов."""
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            if amount_float >= 0:
                continue
            keys.add(_make_key(date_val, str(vendor_val), str(amount_val)))
    wb.close()
    return keys
...
def create_annotated_xls(
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            key = _make_key(date_val, str(vendor_val), str(amount_val))
            if key in csv_keys:
                for cell in row:
...
def reconcile(
    ...
    # записи из CSV, которых НЕТ в XLS
    missing: List[Dict] = []
    for rec in csv_records:
...
"""
    ...
    parser.add_argument('--csv',      required=True, help="Полный путь к CSV")
    parser.add_argument('--xls',      required=True, help="Полный путь к XLS/XLSX")
    parser.add_argument('--out',      default=MISSING_DIR_NAME,
                        help=f"Папка вывода (по умолчанию: {MISSING_DIR_NAME} в cwd)")
    ...
    print("=" * 50)
    reconcile(csv_path=csv_path, xls_path=xls_path, out_dir=out_dir, dry_run=args.dry_run)
    print("=" * 50)
